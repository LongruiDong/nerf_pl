...
# originmodels里的是nerf models是nerf_w
...
def get_opts():
    ...
    parser.add_argument('--root_dir', type=str,
                        default='/media/dlr/nd/nerf_synthetic/lego',
...
@torch.no_grad()
def f(models, embeddings, rays, ts, N_samples, N_importance, chunk, white_back):
...
if __name__ == "__main__":
    args = get_opts()
    # Change here # # 对于nerf_w 这里在原代码上改过了
    encode_appearance = True
    N_a = 48
    encode_transient = True
    N_tau = 16
    beta_min = 0.03 # 0.1 0.03
    N_samples = 64 # 64 128
    N_importance = 64
    N_vocab = 2010 # 2000
    N_emb_xyz = 15
    ...
    if args.dataset_name == 'llff':
        ...
        kwargs['split'] = 'train' # test_train train
    dataset = dataset_dict[args.dataset_name](**kwargs)

    # PosEmbedding for models  Embedding - originmodels
    embedding_xyz = PosEmbedding(N_emb_xyz-1, N_emb_xyz) # 9, 10 14, 15
    ...
    # nerf_fine = NeRF()
    nerf_fine = NeRF('fine',
                 in_channels_xyz=6*N_emb_xyz+3, # add for brand
    ...
    ts = torch.ones(xyz_.shape[0]).cuda().long()
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, B, args.chunk)):
            ...
            inputs = [xyz_embedded, dir_embedded] # inputs for original NeRF
            if encode_appearance:
                inputs += [embedding_a(ts[i:i+chunk])] # (chunk, embed_a_channels) 48
            if encode_transient:
                inputs += [embedding_t(ts[i:i+chunk])] # (chunk, embed_t_channels) 16
            xyzdir_embedded = torch.cat(inputs, 1)
            out_chunks += [nerf_fine(xyzdir_embedded)]
        rgbsigma = torch.cat(out_chunks, 0)

    sigma = rgbsigma[:, 3].cpu().numpy() # -1 -> 3
    ...
    PlyData([PlyElement.describe(vertices_[:, 0], 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'nerfw_{args.scene_name}_{N_samples}.ply')

    # remove noise in the mesh by keeping only the biggest cluster
    print('Removing noise ...')
    mesh = o3d.io.read_triangle_mesh(f"nerfw_{args.scene_name}_{N_samples}.ply")
    ...
    W, H = args.img_wh
    W = W//dataset.img_downscale
    H = H//dataset.img_downscale
    print('img size after scale : {:d},{:d}'.format(W,H))
    focal = dataset.Ks[1][1,1]
    # K = np.array([[focal, 0, W/2],
    #               [0, focal, H/2],
    #               [0,             0,   1]]).astype(np.float32)
    K = dataset.Ks[1].astype(np.float32)
    ...
    # bounds = []
    # for i in range(len(dataset.nears)):
    #     near_i = dataset.nears[i+1]
    #     far_i = dataset.fars[i+1]
    ...
    if args.use_vertex_normal: ## use normal vector method as suggested by the author.
        ...
        for idx in tqdm(range(len(dataset.image_paths))):
            ...
            image = Image.open(os.path.join(args.root_dir,'images',dataset.image_paths[idx+1])).convert('RGB')
    ...
    PlyData([PlyElement.describe(vertex_all, 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'nerfw_{args.scene_name}_color.ply')
