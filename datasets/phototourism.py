...
class PhototourismDataset(Dataset): # 继承了pytorch torch.utils.data.Dataset 的抽象类
    ...
    def read_meta(self):
        ...
        self.scene_name = os.path.basename(tsv)[:-4] # ...burg
        ...
        if self.use_cache:
            ...
            imdata = read_images_binary(os.path.join(self.root_dir, 'dense/sparse/images.bin')) # 其实是colmap的输出
            img_path_to_id = {} # https://colmap.github.io/format.html#images-txt
            for v in imdata.values():
                img_path_to_id[v.name] = v.id # 图像文件名 basename: 其Id  1363
            self.img_ids = []
            self.image_paths = {} # {id: filename} 实际按照tsv拿出有id的 773 张
            for filename in list(self.files['filename']): # 被重建的图片的id pose 名字 相机模型id  对应kpt在图像的位置 对应3d点id
        ...
        if self.use_cache:
            ...
            for id_ in self.img_ids:
                K = np.zeros((3, 3), dtype=np.float32) #都是PINHOLE 
        ...
        if self.use_cache:
            ...
            self.poses = np.linalg.inv(w2c_mats)[:, :3] # (N_images, 3, 4) cam->world
        ...
        if self.use_cache:
            ...
            pts3d = read_points3d_binary(os.path.join(self.root_dir, 'dense/sparse/points3D.bin')) # https://colmap.github.io/format.html#points3d-txt
